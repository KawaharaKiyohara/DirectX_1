...
void Sprite::Draw()
{
	...
	ge->GetD3DDeviceContext()->IASetVertexBuffers(	//頂点バッファを設定。
	...
	ge->GetD3DDeviceContext()->IASetIndexBuffer(	//インデックスバッファを設定。
	...
	ge->GetD3DDeviceContext()->DrawIndexed(	//描画命令。
		6,				//インデックス数。
		0,				//開始インデックス番号。0でいい。
		0				//開始頂点番号。0でいい。
	);
