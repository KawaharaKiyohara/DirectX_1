...
void GameUpdate()
{
	...
	//10.2.5 Hands-On　①　平行移動行列を作って、ティーポットを動かす。
	/*if (GetAsyncKeyState(VK_LEFT)) {
		g_teapotPosition.x -= 2.0f;
	}
	if (GetAsyncKeyState(VK_RIGHT)) {
		g_teapotPosition.x += 2.0f;
	}
	CMatrix mTrans;
	mTrans.MakeTranslation(g_teapotPosition);
	g_teapotModel.m_worldMatrix = mTrans;*/
	//10.2.5 Hands-On　②　回転行列を作って、ティーポットを回転させる。
	...
	g_teapotRotation.Multiply(qAddRot);
	CMatrix mRot;
	mRot.MakeRotationFromQuaternion(g_teapotRotation);
	g_teapotModel.m_worldMatrix = mRot;*/
	//10.2.5 Hands-On　③　拡大行列を作って、ティーポットを拡大させる。
	/*if (GetAsyncKeyState(VK_UP)) {
		g_teapotScale.y += 0.1f;
	}
	if (GetAsyncKeyState(VK_DOWN)) {
		g_teapotScale.y -= 0.1f;
	}
	CMatrix mScale;
	mScale.MakeScaling(g_teapotScale);
	g_teapotModel.m_worldMatrix = mScale;*/
	//10.2.7 Hands-On  ①　平行移動行列と回転行列を乗算して、ティーポットを動かす。
	
	//10.2.7 Hands-On  ②　平行移動と回転の乗算順番を変更すると
	//3Dモデルを描画する。
