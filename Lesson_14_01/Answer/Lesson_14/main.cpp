...
void GameUpdate()
{
	...
	if (GetAsyncKeyState(VK_LEFT)) {		//左のキーが押された。
		mRot.MakeRotationY(CMath::DegToRad(1.0f));
	}
	else if (GetAsyncKeyState(VK_RIGHT)) {	//右のキーが押された。
		mRot.MakeRotationY(CMath::DegToRad(-1.0f));
	}
	...
	rotAxis.Cross(toCameraPos, upAxis);
	rotAxis.Normalize();
	mRot = CMatrix::Identity();
	if (GetAsyncKeyState(VK_UP)) {			//上のキーが押された。
		mRot.MakeRotationAxis(rotAxis, CMath::DegToRad(1.0f));
	}
	else if (GetAsyncKeyState(VK_DOWN)) {	//下のキーが押された。
		mRot.MakeRotationAxis(rotAxis, CMath::DegToRad(-1.0f));
	}
	mRot.Mul(toCameraPos);
